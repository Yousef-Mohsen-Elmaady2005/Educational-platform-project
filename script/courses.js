...
coursesGrid.innerHTML = pageItems.map((course) => `
    <a href="courses2.html" class="course-card cursor-pointer w-full max-w-[380px] sm:max-w-none mx-auto block overflow-hidden rounded-[18px] bg-white text-right shadow-[0_8px_22px_rgba(31,55,88,0.06)] flex flex-col">
      <div class="relative aspect-[4/3] sm:aspect-[27/20] overflow-hidden shrink-0">
        ...
        <button type="button" aria-label="إضافة إلى المفضلة" onclick="event.preventDefault(); event.stopPropagation();" class="absolute left-3 top-3 sm:left-3.5 sm:top-3.5 flex h-8 w-8 items-center justify-center rounded-full bg-white text-[#777] shadow-sm transition hover:scale-105">
