...
// البروفايل الشخصي
  ...
  function saveName() {
    ...
    if (first && last) {
      ...
      document.querySelectorAll('[data-account-name], .hero-account-nav a').forEach((element) => {
        element.textContent = fullName;
      });
      document.querySelectorAll('[data-account-initials]').forEach((element) => {
        element.textContent = fullName.split(/\s+/).map((part) => part[0]).join('').slice(0, 2).toUpperCase();
      });
