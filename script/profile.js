...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (burgerBtn && closeBtn && menu && overlay) {
    const openMenu = () => {
      ...
      document.body.classList.add('overflow-y-hidden'); // Lock background scroll
    };
    ...
    const closeMenu = () => {
      ...
      document.body.classList.remove('overflow-y-hidden'); // Unlock scroll
    };
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (waBtn) {
    // Move the button to the body level to prevent overflow/containment issues in header
    document.body.appendChild(waBtn);
    
    const handleScroll = () => {
      if (window.scrollY > 150) {
        waBtn.classList.remove('translate-y-20', 'opacity-0', 'pointer-events-none');
        waBtn.classList.add('translate-y-0', 'opacity-100', 'pointer-events-auto');
      } else {
        waBtn.classList.add('translate-y-20', 'opacity-0', 'pointer-events-none');
        waBtn.classList.remove('translate-y-0', 'opacity-100', 'pointer-events-auto');
      }
    };
    
    // Check initial position and listen to scroll events
